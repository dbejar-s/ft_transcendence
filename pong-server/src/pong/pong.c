...
#define paddle_boost(paddle) ((paddle.direction == UP) ? GAME_BALL_ANGLE_MOVE_BOOST : (paddle.direction == DOWN) ? -GAME_BALL_ANGLE_MOVE_BOOST \
																												 : 0)
...
static inline void _hit_ball(game *game, const u8 player)
{
	...
	if (game->state.ball.y + GAME_BALL_RADIUS >= paddle_pos - GAME_PADDLE_HEIGHT &&
		game->state.ball.y - GAME_BALL_RADIUS <= paddle_pos + GAME_PADDLE_HEIGHT)
	{
		switch (player)
		{
		case GAME_PLAYER_1:
